...
#     .tag { display:inline-block;background:#3a86ff22;color:#3a86ff;
...
#     .debug-box { background:#1e1e2e;border:1px solid #3a86ff;border-radius:8px;
#                  padding:1rem;font-family:monospace;font-size:0.8rem;
#                  color:#0f0;margin:1rem 0; }
...
#     # Debug mode toggle (fixed - don't assign to session state here)
...
#     """Fetch trending news articles from NewsAPI or return mock data"""
#     if api_key:
...
#                     st.warning(f"NewsAPI returned status {r.status_code}. Using mock data.")
#         except Exception as e:
#             if st.session_state.debug_mode:
#                 st.warning(f"NewsAPI error: {e}. Using mock data.")

#     # Mock data for demo mode
#     mock = [
#         {"title": "AI Surpasses Human Performance in Complex Reasoning Tasks",
#          "description": "Researchers report breakthrough results in multi-step reasoning benchmarks.",
#          "source": "Tech Insider", "url": "#", "published": datetime.now().isoformat(), "image": ""},
#         {"title": "Global Clean Energy Investment Hits Record $1.8 Trillion",
#          "description": "Solar and wind lead the charge as fossil fuel investment declines.",
#          "source": "Energy Daily", "url": "#", "published": datetime.now().isoformat(), "image": ""},
#         {"title": "SpaceX Launches New Starship Mission to Deep Space",
#          "description": "The next generation rocket completes its most ambitious test flight.",
#          "source": "Space News", "url": "#", "published": datetime.now().isoformat(), "image": ""},
#         {"title": "Scientists Discover New Antibiotic That Kills Drug-Resistant Bacteria",
#          "description": "A groundbreaking compound shows effectiveness against superbugs.",
#          "source": "Health Weekly", "url": "#", "published": datetime.now().isoformat(), "image": ""},
#         {"title": "Stock Markets Rally as Inflation Cools to 3-Year Low",
#          "description": "Investors cheer as central banks signal rate cuts ahead.",
#          "source": "Finance Post", "url": "#", "published": datetime.now().isoformat(), "image": ""},
#     ]
#     return mock[:count]
...
#     DEMO = {
#         "hook": f"🔥 Breaking: {article['title'][:60]}...",
#         "body": (article.get("description") or "Stay tuned for the full story."),
#         "cta": "Like & Follow for more trending news! 🔔",
#         "hashtags": ["#trending", "#news", "#viral", "#breakingnews", "#update"],
...
#     try:
#         client = OpenAI(api_key=api_key)
#         prompt = f"""You are a professional short-form video scriptwriter for social media.

# Create a {duration}-second video script for this news article:
# Title: {article['title']}
...
# {{
#   "hook": "Opening 1-2 sentences to grab attention (max 15 words)",
#   "body": "Core story in 3-4 punchy sentences (max 60 words)",
#   "cta": "Call-to-action closing line (max 10 words)",
...
#             messages=[
#                 {"role": "system", "content": "You are a short-form video scriptwriter. Always return valid JSON only."},
...
# def generate_dalle_image(api_key: str, prompt: str, style: str, size="1024x1024") -> Image.Image:
...
#         enhanced_prompt = f"{prompt}. {style_prompts.get(style, 'high quality, detailed')}"
...
# def create_fallback_image(width: int, height: int, text: str, accent_color=(58, 134, 255)) -> Image.Image:
...
#         img = Image.new("RGB", (width, height), (58, 134, 255))
#         return img


# def create_text_overlay(image: Image.Image, text: str) -> Image.Image:
...
#                     text = " ".join(script.get("hashtags", ["#news"]))
#                 img = create_fallback_image(width, height, text)
...
#             img_with_text = create_text_overlay(img, text)
...
#                     progress_callback(frames_processed / total_frames, f"Rendering {section_name}...")
...
# st.markdown("*Trending news → OpenAI script → DALL-E images → Video with overlays*")
# st.divider()
...
# with c1:
#     st.info(f"Category: **{news_category}** | Articles: **{max_articles}**")
# with c2:
#     fetch_btn = st.button("🔍 Fetch News", use_container_width=True, type="primary", key="fetch_news_btn")

# if fetch_btn:
#     with st.spinner("Scraping trending news…"):
...
#             <div class='meta'>📡 {a.get('source', '?')} &nbsp;|&nbsp; 📅 {pub}</div>
#         </div>""", unsafe_allow_html=True)
    
#     if len(st.session_state.articles) > 3:
#         st.caption(f"... and {len(st.session_state.articles) - 3} more articles")
...
# else:
#     article_options = [f"{i+1}. {a['title'][:80]}..." for i, a in enumerate(st.session_state.articles)]
...
#             st.info(" ".join(sc.get("hashtags", ["#news"])))
...
#         if st.button("🎨 Generate Images with DALL-E", type="primary", key="generate_images_btn"):
...
#                 status_text.text(f"Generating {section.upper()} image with DALL-E...")
                
#                 # Create prompt based on section
#                 if section == "hook":
#                     prompt = f"News headline visual: {sc.get('hook', 'breaking news')}, professional news photography"
#                 elif section == "body":
#                     prompt = f"News story illustration: {sc.get('body', 'news event')}, journalistic style"
#                 elif section == "cta":
#                     prompt = f"Call to action: subscribe and follow for news, social media style"
#                 else:
#                     prompt = f"Social media hashtags design: {', '.join(sc.get('hashtags', ['news']))}"
                
#                 # Generate image
#                 img = generate_dalle_image(openai_key, prompt, image_style)
...
#             out_path = str(out_dir / f"{safe_title}.mp4")
...
#             "Task 1 · AI Video Generation Tool &nbsp;|&nbsp; "
#             "Python + Streamlit + OpenAI + DALL-E 3 + OpenCV</div>",
#             unsafe_allow_html=True)
